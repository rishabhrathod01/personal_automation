...
| **Data source** | [Google Doc](https://docs.google.com/document/d/YOUR_DOC_ID/edit) — Day 1–12 table with Lunch/Dinner “Main” dishes |
| **Delivery** | [Whapi.Cloud](https://whapi.cloud/) API → WhatsApp group (group ID and cook phone in config) |
...
- [`scripts/meal-plan/config.ts`](scripts/meal-plan/config.ts) — Doc URL, Whapi group ID, cook phone, chapati defaults, rice days. Token comes from GitHub Secrets only.
...
**Setup:** One-time steps (Whapi token, Google Doc sharing, GitHub secret, verify) are in **[SETUP.md](SETUP.md)**.
...
3. **Run**
   ...
   - **Local:** `npm run meal-plan:send` (needs `WHAPI_API_TOKEN` in env). List groups: `WHAPI_API_TOKEN=… npm run meal-plan:get-groups`.
...
- **Secrets:** Only `WHAPI_API_TOKEN` is stored in GitHub; group ID and cook phone are in `config.ts`.
